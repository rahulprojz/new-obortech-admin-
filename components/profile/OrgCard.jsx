...
function OrgCard(props) {
    ...
    const { icon, heading, stateRegId, caption, idData, regData, showVerified, approverOrgs, userData, from = 'orgApprovalModal' } = props
    ...
    const isNotSeniorManager = userData?.role_id != process.env.ROLE_SENIOR_MANAGER
    ...
    return (
        <>
            ...
            {userData &&
                (mvsVerifiedUser && !mvsVerifiedOrg && !isNotCEOUser && !isNotSeniorManager ? (
                    <Button
                        ...
                        disabled={(showVerified == '1' ? (mvsVerifiedUser ? mvsVerifiedOrg || isNotCEOUser || isNotSeniorManager : true) : mvsVerifiedUser) || from != 'profile'}
                    ...
                    <div style={{ margin: '0 0 20px 35px' }}>
                        <img className='mx-1' src='/static/img/not-verify.png' alt='not-verified' /> {string.mvsNotVerifiedOrg}
                    </div>
                ))}
        </>
    )
