...
function CommitModal({ committingProposal, onProposalCommitted, smartContract, commitProposalId, selectedTab }) {
                ...
                <div className='modal-content'>
                    <div className='modal-header ob-justify-center pt-4 border-0'>
                        <h5 className='modal-title text-dark pt-3 text-uppercase font-weight-bold' id='exampleModalLabel'>
                            {string.smartContract.commitConfirmation}
                        </h5>
                    ...
                    <div className='modal-body ob-modal-padding'>
                        <Formik
                            ...
                            onSubmit={async (values, { resetForm }) => {
                                const isCancelled = await onProposalCommitted(smartContract.proposal.name, smartContract.proposal.version, commitProposalId, selectedTab)
                                if (isCancelled) {
                                    resetForm()
                                }
                            }}
                            ...
                            {({ errors, touched, handleChange, handleSubmit, values }) => (
                                <form className='form-container' onSubmit={handleSubmit}>
                                    <div className='modal-footer border-0'>
                                        ...
                                        <LoaderButton cssClass='default-css btn btn-primary btn-md ob-min-w140 text-uppercase' type='submit' isLoading={committingProposal} text={string.smartContract.yes} />
                                    </div>
                                </form>
                            )}
                        </Formik>
