...
const SmartContractPage = (props) => {
    ...
    useEffect(() => {
        if (!isOpenGithubModal) {
            if (proposal) {
                viewSmartContractDetails(proposal[0], proposal[1], cancelProposalId, true)
                if (proposal[2]) {
                    switch (proposal[2]) {
                        case 'approvals':
                            setSelectedSubTab(SMART_CONTRACT_PAGE_TAB.APPROVALS)
                            handleFetchSubTab(SMART_CONTRACT_PAGE_TAB.APPROVALS, proposal[0], proposal[1])
                            break
                        case 'source-code-access':
                            setSelectedSubTab(SMART_CONTRACT_PAGE_TAB.CODE_ACCESS)
                            handleFetchSubTab(SMART_CONTRACT_PAGE_TAB.CODE_ACCESS, proposal[0], proposal[1])
                            break
                        case 'comments':
    ...
    const handleFetchList = async (params = {}) => {
        try {
            if (params.selectedTab === SMART_CONTRACT_PAGE_TAB.SMART_CONTRACT_LISTING) {
                ...
                setCancelProposalId(proposalId)
                ...
                commentResponse.data.proposalId = commentResponse.data.id
                setCancelProposalId(commentResponse.data.id)
    ...
    const viewDetails = async (version, name) => {
        ...
        try {
            ...
            const commentQuery = { ...commentQueryData }
            const commentResponse = await fetchSmartContractComments(name, commentQueryData, cookies.authToken)
            const data = {
                orgName: user.organization.blockchain_name.toLowerCase(),
                userName: user.unique_id.toLowerCase(),
                page_size: '1000',
            ...
            const query = { ...data }
            const response = await fetchSmartContracts(query, cookies.authToken)
            const smartContractData = response.data.data.find((contractData) => contractData.data.name == proposal[0])
            commentResponse.data.proposalId = commentResponse.data.id
            setCommentsData(commentResponse.data.comments)
            setSmartContract(commentResponse.data)
            setCancelProposalId(smartContractData.id)
            setIsLoading(false)
            setLoadingComments(false)
            setLoadingApprovals(false)
    ...
    const handleFetchSubTab = async (tab, name, version) => {
        ...
        try {
            if (tab === SMART_CONTRACT_PAGE_TAB.COMMENTS) {
                setSelectedSubTab(SMART_CONTRACT_PAGE_TAB.COMMENTS)
                if (router.asPath != '/smart-contracts/' + name + '/' + version + '/comments') {
                    router.push('/smart-contracts/' + name + '/' + version + '/comments')
                }
                setLoadingComments(true)
                const commentQueryData = {
                    orgName: user.organization.blockchain_name.toLowerCase(),
                ...
                setSelectedSubTab(SMART_CONTRACT_PAGE_TAB.CODE_ACCESS)
                if (router.asPath != '/smart-contracts/' + name + '/' + version + '/source-code-access') {
                    router.push('/smart-contracts/' + name + '/' + version + '/source-code-access')
                }
                ...
                setSelectedSubTab(SMART_CONTRACT_PAGE_TAB.APPROVALS)
                if (router.asPath != '/smart-contracts/' + name + '/' + version + '/approvals') {
                    router.push('/smart-contracts/' + name + '/' + version + '/approvals')
                }
                setLoadingApprovals(true)
    ...
    const createNewProposal = async (values) => {
        ...
        try {
            ...
            handleFetchList({ selectedTab: SMART_CONTRACT_PAGE_TAB.SMART_CONTRACT_DETAILS })
            router.push('/smart-contracts/' + values.proposal_name)
            setAddingProposal(false)
    ...
    return (
        <div className='pb-4'>
            ...
            <div className='modal fade document' id='editModal' tabIndex='-1' role='dialog' aria-labelledby='exampleModalLabel' aria-hidden='true'>
                <EditModal addingProposal={addingProposal} loadingProposal={loadingProposal} smartContract={smartContractEditData.proposal} addProposal={addProposal} />
            </div>
            {showModal && (
